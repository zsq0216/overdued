...
struct word
{
    ...
    char pos;
    double frq;
    word(string k, char p, double f) : key(k), pos(p), frq(f) {}
};
...
public:
    ...
    void countnum(Node *p);
};
// if child is full and the parent is not full, split the child.
void BTree::SplitChild(Node *x, const int i)
{ // O(t)
    Node *y = x->pToChild[i];
    Node *z = new Node;
...
void BTree::remove(Node *x, const int k)
{ // This function guarantees x->key.size() >= t,except root
    int i = 0;
    while (i < x->key.size() && x->key[i] < k)
        ++i;
    if (i < x->key.size() && x->key[i] == k)
    {
        if (x->pToChild.empty()) //怎么能保证这一步不会让n<t-1
            x->key.erase(x->key.begin() + i);
        else
        {
            if (x->pToChild[i]->key.size() >= t) //预看子节点
            {
